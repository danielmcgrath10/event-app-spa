...
export default function EventView({ events, session }) {
  ...
  const populateInvites = () => {
    return event.invites.map((invite, index) => (
      <tr key={index}>
        <td>{invite.email}</td>
        <td>{invite.url}</td>
        <td>{invite.accept.toString()}</td>
        {event.user.id === session.user_id ? (
          <td>
            <a className={"click-elem"} onClick={() => handleDelInv(invite.id)}>
              x
            </a>
          </td>
        ) : null}
          ...
          <Row id={"comment-row"}>
            ...
            <Col className={"event-row"}>
              ...
              {event.user.id === session.user_id ? (
                ...
                <Row>
                  <Col>
                    <Button onClick={() => handleUpdateInvite(true)}>
                      Accept
                    </Button>
                  </Col>
                  <Col>
